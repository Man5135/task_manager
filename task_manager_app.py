...
class TaskManagerApp:
    ...
    def _setup_performance_tab(self):
        """Sets up the performance tab with machine sensor simulation."""
        ...
        # Main frame for machine simulation
        self.machine_frame = tk.Frame(self.performance_tab, bg=self.dark_blue)
        self.machine_frame.pack(fill="both", expand=True, padx=10, pady=10)

        # System status frame (CPU, Memory, Disk)
        self.system_info_frame = tk.Frame(self.machine_frame, bg=self.dark_blue)
        self.system_info_frame.pack(fill="x", pady=5)

        self.cpu_label = tk.Label(self.system_info_frame, 
                                 text=self.translate("cpu_label", cpu_percent=0),
                                 bg=self.dark_blue, fg=self.light_text)
        self.cpu_label.pack(side="left", padx=10)

        self.memory_label = tk.Label(self.system_info_frame,
                                   text=self.translate("memory_label", memory_percent=0, 
        ...
        self.disk_label = tk.Label(self.system_info_frame,
                                 text=self.translate("disk_label", disk_percent=0, 
                                                    disk_used_gb=0, disk_total_gb=0),
                                 bg=self.dark_blue, fg=self.light_text)
        self.disk_label.pack(side="left", padx=10)

        # Machine sensors simulation frame
        self.sensors_frame = tk.LabelFrame(self.machine_frame, 
                                         text="Датчики станка", 
                                         bg=self.dark_blue, fg=self.light_text)
        self.sensors_frame.pack(fill="both", expand=True, pady=10)

        # Create sensor indicators
        self._create_sensor_indicators()
    ...
    def _create_sensor_indicators(self):
        """Creates visual indicators for machine sensors."""
        # Temperature sensor
        self.temp_frame = tk.Frame(self.sensors_frame, bg=self.dark_blue)
        self.temp_frame.pack(fill="x", pady=5)
        
        tk.Label(self.temp_frame, text="Температура (°C):", 
                bg=self.dark_blue, fg=self.light_text).pack(side="left", padx=5)
        
        self.temp_value = tk.Label(self.temp_frame, text="25.0", 
                                  bg=self.dark_blue, fg=self.light_text)
        self.temp_value.pack(side="left", padx=5)
        
        self.temp_gauge = Canvas(self.temp_frame, width=200, height=20, bg=self.dark_blue, 
                                highlightthickness=0)
        self.temp_gauge.pack(side="left", padx=5)
        self.temp_gauge.create_rectangle(0, 0, 100, 20, fill="green", outline="")
# Pressure sensor
        self.pressure_frame = tk.Frame(self.sensors_frame, bg=self.dark_blue)
        self.pressure_frame.pack(fill="x", pady=5)
        
        tk.Label(self.pressure_frame, text="Давление (бар):", 
                bg=self.dark_blue, fg=self.light_text).pack(side="left", padx=5)
        
        self.pressure_value = tk.Label(self.pressure_frame, text="1.0", 
                                      bg=self.dark_blue, fg=self.light_text)
        self.pressure_value.pack(side="left", padx=5)
        
        self.pressure_gauge = Canvas(self.pressure_frame, width=200, height=20, 
                                    bg=self.dark_blue, highlightthickness=0)
        self.pressure_gauge.pack(side="left", padx=5)
        self.pressure_gauge.create_rectangle(0, 0, 100, 20, fill="blue", outline="")

        # Vibration sensor
        self.vibration_frame = tk.Frame(self.sensors_frame, bg=self.dark_blue)
        self.vibration_frame.pack(fill="x", pady=5)
        
        tk.Label(self.vibration_frame, text="Вибрация (мм/с):", 
                bg=self.dark_blue, fg=self.light_text).pack(side="left", padx=5)
        
        self.vibration_value = tk.Label(self.vibration_frame, text="0.1", 
                                       bg=self.dark_blue, fg=self.light_text)
        self.vibration_value.pack(side="left", padx=5)
        
        self.vibration_gauge = Canvas(self.vibration_frame, width=200, height=20, 
                                     bg=self.dark_blue, highlightthickness=0)
        self.vibration_gauge.pack(side="left", padx=5)
        self.vibration_gauge.create_rectangle(0, 0, 10, 20, fill="yellow", outline="")

        # Spindle speed sensor
        self.spindle_frame = tk.Frame(self.sensors_frame, bg=self.dark_blue)
        self.spindle_frame.pack(fill="x", pady=5)
        
        tk.Label(self.spindle_frame, text="Скорость шпинделя (об/мин):", 
                bg=self.dark_blue, fg=self.light_text).pack(side="left", padx=5)
        
        self.spindle_value = tk.Label(self.spindle_frame, text="0", 
                                     bg=self.dark_blue, fg=self.light_text)
        self.spindle_value.pack(side="left", padx=5)
        
        self.spindle_gauge = Canvas(self.spindle_frame, width=200, height=20, 
                                   bg=self.dark_blue, highlightthickness=0)
        self.spindle_gauge.pack(side="left", padx=5)
        self.spindle_gauge.create_rectangle(0, 0, 0, 20, fill="cyan", outline="")

        # Load sensor
        self.load_frame = tk.Frame(self.sensors_frame, bg=self.dark_blue)
        self.load_frame.pack(fill="x", pady=5)
        
        tk.Label(self.load_frame, text="Нагрузка (%):", 
                bg=self.dark_blue, fg=self.light_text).pack(side="left", padx=5)
        
        self.load_value = tk.Label(self.load_frame, text="0.0", 
                                  bg=self.dark_blue, fg=self.light_text)
        self.load_value.pack(side="left", padx=5)
        
        self.load_gauge = Canvas(self.load_frame, width=200, height=20, 
                                bg=self.dark_blue, highlightthickness=0)
        self.load_gauge.pack(side="left", padx=5)
        self.load_gauge.create_rectangle(0, 0, 0, 20, fill="magenta", outline="")

    def simulate_sensors(self):
        """Machine sensor readings with random variations."""
        # Temperature: 20-80°C (normal 25-50, warning 50-70, danger >70)
        self.sensor_values["temperature"] += random.uniform(-0.5, 0.5)
        self.sensor_values["temperature"] = max(20, min(80, self.sensor_values["temperature"]))
        
        # Pressure: 0.5-5.0 bar (normal 1-4, warning <1 or >4)
        self.sensor_values["pressure"] += random.uniform(-0.05, 0.05)
        self.sensor_values["pressure"] = max(0.5, min(5.0, self.sensor_values["pressure"]))
# Vibration: 0-10 mm/s (normal 0-2, warning 2-5, danger >5)
        self.sensor_values["vibration"] += random.uniform(-0.1, 0.1)
        self.sensor_values["vibration"] = max(0, min(10, self.sensor_values["vibration"]))
        
        # Spindle speed: 0-10000 rpm (simulate acceleration/deceleration)
        if random.random() < 0.1:  # 10% chance to change direction
            self.sensor_values["spindle_speed"] = random.randint(0, 10000)
        else:
            change = random.randint(-200, 200)
            self.sensor_values["spindle_speed"] += change
            self.sensor_values["spindle_speed"] = max(0, min(10000, self.sensor_values["spindle_speed"]))
        
        # Load: 0-100% (simulate varying load)
        self.sensor_values["load"] += random.uniform(-5, 5)
        self.sensor_values["load"] = max(0, min(100, self.sensor_values["load"]))

        # Update UI
        self._update_sensor_ui()
        
        # Schedule next update
        self.master.after(500, self.simulate_sensors)

    def _update_sensor_ui(self):
        """Updates the sensor UI elements with current values."""
        # Temperature
        temp = self.sensor_values["temperature"]
        self.temp_value.config(text=f"{temp:.1f}")
        temp_width = int((temp - 20) * 3.33)  # Scale 20-80°C to 0-200 pixels
        self.temp_gauge.delete("all")
        if temp > 70:
            color = "red"
        elif temp > 50:
            color = "orange"
            ...
            color = "green"
        self.temp_gauge.create_rectangle(0, 0, temp_width, 20, fill=color, outline="")

        # Pressure
        pressure = self.sensor_values["pressure"]
        self.pressure_value.config(text=f"{pressure:.2f}")
        pressure_width = int((pressure - 0.5) * 44.44)  # Scale 0.5-5.0 to 0-200 pixels
        self.pressure_gauge.delete("all")
        if pressure < 1 or pressure > 4:
            color = "orange"
        else:
            color = "blue"
        self.pressure_gauge.create_rectangle(0, 0, pressure_width, 20, fill=color, outline="")

        # Vibration
        vibration = self.sensor_values["vibration"]
        self.vibration_value.config(text=f"{vibration:.2f}")
        vibration_width = int(vibration * 20)  # Scale 0-10 to 0-200 pixels
        self.vibration_gauge.delete("all")
        if vibration > 5:
            color = "red"
        elif vibration > 2:
            color = "orange"
            ...
            color = "yellow"
        self.vibration_gauge.create_rectangle(0, 0, vibration_width, 20, fill=color, outline="")

        # Spindle speed
        speed = self.sensor_values["spindle_speed"]
        self.spindle_value.config(text=f"{speed}")
        speed_width = int(speed / 50)  # Scale 0-10000 to 0-200 pixels
        self.spindle_gauge.delete("all")
        self.spindle_gauge.create_rectangle(0, 0, speed_width, 20, fill="cyan", outline="")

        # Load
        load = self.sensor_values["load"]
        self.load_value.config(text=f"{load:.1f}")
        load_width = int(load * 2)  # Scale 0-100% to 0-200 pixels
        self.load_gauge.delete("all")
        if load > 90:
            color = "red"
        elif load > 70:
            color = "orange"
            ...
            color = "magenta"
        self.load_gauge.create_rectangle(0, 0, load_width, 20, fill=color, outline="")
